...
public class SearchModeSearchDatabaseAuthenticationHandler extends AbstractJdbcUsernamePasswordAuthenticationHandler
    ...
    protected final HandlerResult authenticateUsernamePasswordInternal(final UsernamePasswordCredential credential)
        ...
        final String password = credential.getPassword().trim();
        final String encyptedPassword = getPasswordEncoder().encode(password).trim().toUpperCase();
        logger.info("=== encyptedPassword = {}", encyptedPassword);
        final int count;
        try {
            logger.info("=== db password begin ===");
            logger.info(this.sql);
            count = getJdbcTemplate().queryForObject(this.sql, Integer.class, username, encyptedPassword);
            logger.info("=== db password over ===");
